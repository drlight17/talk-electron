    ...
    try {
      ...
      function openPopup(url) {
        ...
        if (url.includes('/settings/user')) {
          title = "Настройки пользователя" + " - " + store.get('server_url');
          allow_navi = true;
        } else if (url.includes('/u/'))  {
          title = "Профиль" + " - " + store.get('server_url')
        } else {
          title = "Помощь" + " - " + store.get('server_url')
        }
      ...
      async function createWindow () {
        ...
        win = new BrowserWindow({
          title: app.getName() + " - " + store.get('server_url'),
          center: true,
      ...
      function guiInit() {
        ...
        appIcon.setToolTip(app.getName()+ " - " + store.get('server_url') + " - Загрузка...");
      ...
      function setAllowDomains () {
        ...
        dialog.showMessageBox(win, {
            'type': 'question',
            'title': 'Вопрос об SSO',
            'message': "Планируется ли использование механизма SSO при авторизации в Nextcloud?",
            'buttons': [
                'Да',
                'Нет'
            ]
        ...
        .then((result) => {
            ...
            if (result.response === 0) {
              ...
              prompt({
                title: 'Разрешенные домены Nextcloud',
                label: 'Введите разрешенные домены для авторизации <br> на сервере Nextcloud через запятую <br>(wildcard поддерживается *):',
                useHtmlLabel: true,
                buttonLabels: '',
